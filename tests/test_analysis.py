...
def test_pipeline_reproduces_committed_statistics():
    analysis.main()
    values = read_summary()
    assert np.isclose(values["depth_ppm"], 8749.24883751, rtol=0, atol=0.1)
    assert np.isclose(values["chi_square_flat"], 32632.6043534, rtol=1e-9)
    assert np.isclose(values["chi_square_box"], 21892.5410266, rtol=1e-9)
    assert np.isclose(values["delta_chi_square"], 10740.0633268, rtol=1e-9)
