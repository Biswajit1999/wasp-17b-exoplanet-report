"""Transit-window analysis of a real TESS SPOC light curve for WASP-17 b.
...
DATA_FILE = DATA_DIR / "tess2019140104343-s0012-0000000066818296-0144-s_lc.fits"
PLANET = "WASP-17 b"
PERIOD_DAYS = 3.73543
EPOCH_BJD = 2454592.80154
DURATION_HOURS = 4.4232
SECTOR = 12
FIGURE_FILE = FIG_DIR / "wasp17b_tess_transit.png"
...
def main() -> dict[str, float]:
    ...
    ax.scatter(hours, (result["flux"] - 1) * 1e6, s=5, alpha=0.12, color="#9c36b5", label="quality-filtered cadences")
    ...
    ax.plot(hours[order], (result["model"][order] - 1) * 1e6,
            color="#9c36b5", linewidth=2.2, label="fitted box model")
    ax.axvspan(-DURATION_HOURS / 2.0, DURATION_HOURS / 2.0, color="#9c36b5", alpha=0.08)
